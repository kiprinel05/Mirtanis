...
@Component({
  ...
  template: `
    <div class="fc-wrap" [class.fc-wrap--show]="visible()">
      <!-- WhatsApp -->
      <a [href]="waLink" target="_blank" rel="noopener" class="fc-btn fc-btn--wa" aria-label="Scrie-ne pe WhatsApp">
  ...
  styles: [`
    ...
    .fc-wrap--show { opacity: 1; transform: none; pointer-events: auto; }
    @media (min-width: 640px) { .fc-wrap { bottom: 24px; right: 24px; } }
    ...
    .fc-btn:hover { transform: translateY(-3px); filter: brightness(1.06); }
    .fc-btn:active { transform: scale(.95); }
    .fc-btn--wa   { background: linear-gradient(135deg, #25D366, #128C7E); }
    .fc-btn--call { background: linear-gradient(135deg, #e4c071, #cda24b); color: #2a1f0e; }

    /* attention pulse on WhatsApp */
    .fc-btn--wa { position: relative; }
    .fc-btn--wa::after {
...
export class FloatingContactComponent {
  ...
  readonly waLink = 'https://wa.me/407XXXXXXXX';
  readonly telLink = 'tel:+407XXXXXXXX';
